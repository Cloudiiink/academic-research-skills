...
- **Guided-writing mode for `academic-paper`.** New mentor-type writing mode that guides users through section-by-section drafting via Socratic dialogue, option presentation, and dynamic feedback rather than auto-generating full text. Includes: `guided_mode_protocol.md` (6-step flow), `guided_mentor_agent.md` (5-round per-section protocol with commitment question, 2–3 options mechanism, draft label, and max-3 feedback restraint), `guided_reviewer_agent.md` (5-class dynamic triggering with detection heuristics and severity-priority ordering), `ars-guided-writing.md` command file, and `guided_mode_example.md` walkthrough. SKILL.md updated to v3.1.3 with 11 modes, trigger keywords in EN / zh-TW / zh-CN, and activation rules. MODE_REGISTRY.md updated. CI lint `scripts/check_guided_mode_protocol.py` + 22 unit tests across 4 test files, all wired into `_ci_pytest_manifest.toml` and `spec-consistency.yml`. Zero existing agent files modified.
